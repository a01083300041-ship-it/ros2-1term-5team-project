...
class LineFollower2(Node):
    ...
    def __init__(self, line_tracker: LineTracker):
        ...
        self._subscription = self.create_subscription(Image, '/camera2/image_raw', self.image_callback, 10)  # auto create topic name
        self._subscription2 = self.create_subscription(Image, '/camera4/image_raw', self.image_callback2, 10)  # auto create topic name

        self.lidar_subscription = self.create_subscription(LaserScan, '/scan2', self.scan_callback, 10)

        self._publisher = self.create_publisher(Twist, 'cmd_vel', 2)  # create publisher
        self.twist = Twist()
...
def main():
    rclpy.init()
    tracker = LineTracker()
    follower = LineFollower2(tracker)
    try:
        rclpy.spin(follower)
